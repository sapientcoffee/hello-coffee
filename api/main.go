...
func rating(w http.ResponseWriter, r *http.Request) {
	...
	func rating(w http.ResponseWriter, r *http.Request) {
		...
		// Read the coffee document from Firestore
		...
		var coffee Coffee
		if err := doc.DataTo(&coffee); err != nil {
			http.Error(w, "Error parsing data from Firestore", http.StatusInternalServerError)
			return
		}
		// Return the rating
		fmt.Fprintf(w, "%v", coffee.Rating)
	}
